...
@Slf4j
@ControllerAdvice(basePackages = "com.sherpa.exambank")
...
//    }
    ...
    protected ResponseEntity<ErrorResponse> handleHttpRequestMethodNotSupportedException(HttpRequestMethodNotSupportedException e) {
        ...
        log.info("METHOD_NOT_ALLOWED 호출");
        return ResponseEntity.status(response.getStatus()).body(response);
    }
    ...
    protected ResponseEntity<ErrorResponse> handleCustomException(CustomException e) {
        ErrorResponse response = new ErrorResponse(ErrorCode.RESOURCE_NOT_FOUND); // CustomException에 ErrorCode Enum 반환
        log.info("ExceptionHandler 호출 = {} ",response.getCode());
        return ResponseEntity.status(response.getStatus()).body(response);
    }
